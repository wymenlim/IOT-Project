...
def print_report(server_stats: Stats, player_stats_list: list):
    ...
    # Attempt to split relay vs non-relay based on hop count in rounds for any player in that round
    # Split press data by hop count across all rounds
    direct_times = []
    relay_times  = []
    direct_ok    = 0
    direct_total = 0
    ...
    for r in server_stats.rounds:
        for pidx, pdata in r.presses.items():
            if pdata['hop'] == 0:
                direct_times.append(pdata['reaction_ms'])
                direct_ok += 1
            else:
                relay_times.append(pdata['reaction_ms'])
                relay_ok += 1
            direct_total += (1 if pdata['hop'] == 0 else 0)
            relay_total  += (1 if pdata['hop'] >= 1 else 0)

    def fmt_lat(times):
        if not times:
            return "N/A"
        return f"{round(statistics.mean(times), 1)} ms avg (n={len(times)})"

    def fmt_psr(ok, total):
        if total == 0:
            return "N/A"
        return f"{100*ok//total}% ({ok}/{total} presses)"

    print(f"  {'Packet Success Rate':<35} {fmt_psr(direct_ok, direct_total):>20}  {fmt_psr(relay_ok, relay_total):>20}")
    print(f"  {'Mean Reaction Latency':<35} {fmt_lat(direct_times):>20}  {fmt_lat(relay_times):>20}")
    print(f"  {'ACK Timeouts (player-side)':<35} {'(see player logs)':>20}  {'(see player logs)':>20}")
    print(f"  {'Auth / Replay Drops':<35} {server_stats.auth_failures + server_stats.replay_drops:>20}  {'':>20}")
    print()
